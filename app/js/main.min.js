...
const inputRub=document.querySelector("#rub"),inputUsd=document.querySelector("#usd");inputRub.addEventListener("input",(()=>{const e=new XMLHttpRequest;e.open("GET","js/current.json"),e.setRequestHeader("Content-type","application/json charset=utf-8"),e.send(),e.addEventListener("readystatechange",(()=>{if(4===e.readyState&&200===e.status){console.log(e.response);const t=JSON.parse(e.response);inputUsd.value=(+inputRub.value/t.current.usd).toFixed(2)}else inputUsd.value="Что-то пошло не так!"}))}));